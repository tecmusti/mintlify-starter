---
title: "POST /api-keys — Create an API Key"
sidebarTitle: "POST /api-keys"
description: "POST /api-keys — create a new estudjo API key. The full key is shown only once in the response. Requires an existing X-Api-Key."
---

Create a new API key for your estudjo account. You might use additional keys to isolate different integrations, rotate credentials, or grant scoped access to separate services. The full key value is returned **only once** in the creation response — store it immediately.
...
```json
{
  ...
  "data": {
    ...
    "api_key": "9e6e81a6d82cffc504763463134cf2606c383eac6eac4c99",
