---
title: "POST /generate — Queue an AI Generation"
...
<Tabs>
  <Tab title="Image">
    ...
    <ParamField body="scene" type="string" required>
      Scene UUID. Provide either `scene` or `sceneId` — one of the two is required.
    </ParamField>

    <ParamField body="sceneId" type="string" required>
      Alternative field name for the scene UUID. Interchangeable with `scene`.
    </ParamField>
    ...
    <ParamField body="additional_data" type="object">
      Flat object of `string:string` pairs prepended to the prompt. Send `{}` or omit this field entirely to skip.
    </ParamField>
  ...
  <Tab title="Video">
    ...
    <ParamField body="prompt" type="string" required>
      Direct prompt string describing the desired video output. Required unless `scene` or `sceneId` is provided. When `prompt` is present, `scene`/`sceneId` is ignored entirely.
    </ParamField>

    <ParamField body="scene" type="string" required>
      Scene UUID. Used only when `prompt` is absent. Provide either `scene` or `sceneId`.
    </ParamField>

    <ParamField body="sceneId" type="string" required>
      Alternative field name for the scene UUID. Used only when `prompt` is absent.
    </ParamField>
...
| `provider_error` | 502 | The upstream AI provider returned an error |
