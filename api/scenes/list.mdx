...
GET /scenes returns both predefined (estudjo-provided) and user-created scenes. Only `name`, `scene_id`, and `type` are returned here — use [GET /scenes/\{scene\_id\}](/api/scenes/get) for full details of your own scenes.
...
```text
GET https://api.estudjo.com/scenes
```
...
```bash
curl 'https://api.estudjo.com/scenes?category=womens-clothing&type=user_created&limit=50' \
