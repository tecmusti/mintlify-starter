---
title: "Generate AI Product Images with estudjo"
...
|---|---|---|
| `model` | string | Model identifier (e.g. `nano-banana`) |
| `category` | string | Category slug (e.g. `womens-clothing`) |
| `aspect-ratio` | string | e.g. `1:1`, `16:9` |
| `resolution` | string | e.g. `1K` |
| `scene` or `sceneId` | string (UUID) | The scene to use |
| `image_url` | string (URL) | Publicly accessible product image |

## Optional Fields

**`additional_data`** — a flat object of `string:string` pairs that are injected into the generation prompt as `key:value` lines. Use this to provide product-specific context such as names, materials, or colours.
