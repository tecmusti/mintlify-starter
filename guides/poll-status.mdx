---
title: "Poll Generation Status and Retrieve Results"
sidebarTitle: "Poll Status"
description: "Use GET /status to track an estudjo generation from queued to completed and download the finished image or video from result_urls."
---
...
The URLs in `result_urls` are **permanent, public-read** links hosted on estudjo's own object storage. You can use them directly in your application (e.g. in an `<img>` or `<video>` tag) or download the files and store them in your own infrastructure — both are supported.
