...
The URLs in `result_urls` are **permanent, public-read** links hosted on estudjo's own object storage. Do not use them directly in your application (e.g. in an `<img>` or `<video>` tag), download the files and store them in your own infrastructure. Generated files are available for downloading for 72 hours. 
